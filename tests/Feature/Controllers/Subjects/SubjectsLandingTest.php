...
class SubjectsLandingTest extends TestCase
{
    use DatabaseTransactions;

    protected function index(): string
    {
        return route('subjects.subjects.index');
    }
    ...
    public function test_landing_with_campus(): void
    {
        $admin = Person::where('email', 'fablms@kalinec.net')->first();
        $campus = Campus::find(2); // Try another campus

        $response = $this->actingAs($admin)
            ->get(route('subjects.subjects.index', ['campus' => $campus->id]));

        $response->assertStatus(Response::HTTP_OK);
        $response->assertSee($campus->name);
    }
}
