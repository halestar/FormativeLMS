...
namespace Feature\Controllers\SchoolSettings;
...
class SchoolSettingsLandingTest extends TestCase
{
	use DatabaseTransactions;

	protected function index(): string
	{
		return route('settings.school');
	}

	protected function names(): string
	{
		$role = SchoolRoles::EmployeeRole();
		return route('settings.school.name', $role);
	}
	 ...
	 * names
	 */
	public function test_names_admin(): void
	{
		$admin = Person::where('email', 'fablms@kalinec.net')->first();
		$response = $this->actingAs($admin)
			->get($this->names());
	...
	public function test_names_staff(): void
	{
		$staff = Person::where('email', 'staff@kalinec.net')->first();
		$response = $this->actingAs($staff)
			->get($this->names());
	...
	public function test_names_faculty(): void
	{
		$faculty = Person::where('email', 'faculty@kalinec.net')->first();
		$response = $this->actingAs($faculty)
			->get($this->names());
	...
	public function test_names_student(): void
	{
		$student = Person::where('email', 'student@kalinec.net')->first();
		$response = $this->actingAs($student)
			->get($this->names());
	...
	public function test_names_parent(): void
	{
		$parent = Person::where('email', 'parent@kalinec.net')->first();
		$response = $this->actingAs($parent)
			->get($this->names());
	...
	public function test_names_guest(): void
	{
		$response = $this->actingAsGuest()
			->get($this->names());
		$response->assertRedirect(route('login'));
