...
<form action="{{ route('settings.school.update.classes') }}" method="POST">
    ...
    <div class="school-setting mb-3">
        <label class="form-label" aria-describedby="force_class_managementHelp">{{ __('system.settings.classes.management') }}</label>
        <div class="form-check">
            ...
            />
            <label class="form-check-label" for="force_class_management_1">{{ __('system.settings.classes.management.force') }}</label>
        </div>
        <div class="form-check">
            <input
                    type="radio"
                    ...
                    @checked(!$schoolSettings->force_class_management)
            />
            <label class="form-check-label" for="force_class_management_0">{{ __('system.settings.classes.management.choice') }}</label>
        </div>
        <x-utilities.error-display key="force_class_management">{{ $errors->first('force_class_management') }}</x-utilities.error-display>
        <div id="force_class_managementHelp" class="form-text">{{ __('system.settings.classes.management.help') }}</div>
    </div>

    <div class="school-setting mb-3">
        <label class="form-label" for="class_management_connection_id">{{ __('system.settings.classes.management.system') }}</label>
        <select
            class="form-select"
        ...
        <x-utilities.error-display key="class_management_connection_id">{{ $errors->first('class_management_connection_id') }}</x-utilities.error-display>
        <div id="class_management_connection_idHelp" class="form-text">{{ __('system.settings.classes.management.system.help') }}</div>
    </div>
