...
<h2 style="margin:0 0 12px 0;font-size:20px;color:#212529;">Coverage Confirmed</h2>

<p style="margin:0 0 16px 0;color:#212529;">
    You are now scheduled to cover <strong>{{ $subRequest->requester_name }}</strong>
    (<a href="mailto:{{ $subRequest->requester->email }}" style="color:#0d6efd;text-decoration:none;">{{ $subRequest->requester->email }}</a>)
    on <strong>{{ $subRequest->requested_for->format('m/d/Y') }}</strong>
    from <strong>{{ $subRequest->subStartTime($sub)->format('h:i A') }}</strong> to
    <strong>{{ $subRequest->subEndTime($sub)->format('h:i A') }}</strong>.
</p>

<p style="margin:0 0 8px 0;font-weight:600;color:#212529;">Classes You Are Covering</p>
...
<p style="margin:0;color:#495057;">
    Thank you for covering this request.
</p>
