        ...
        <form action="{{ route('locations.rooms.store') }}" method="POST">
            ...
            @else
                <div class="input-group mb-3">
                    <label class="input-group-text" for="area">{{ __('locations.rooms.location', ['building' => $building->name]) }}</label>
                    <select name="area_id" id="area" class="form-select @error('area_id') is-invalid @enderror" required>
