            ...
            <div class="input-group w-50">
                ...
                >
                    @foreach(Auth::user()->employeeCampuses as $campusOption)
                        <option value="{{ $campusOption->subjects()->first()->id }}" @if($campusOption->id == $subject->campus_id) selected @endif>{{ $campusOption->name }}</option>
                    @endforeach
                </select>
                <select
                    id="subject_id"
                ...
                >
                    @foreach($subject->campus->subjects as $subjectOpts)
                        <option value="{{ $subjectOpts->id }}" @if($subjectOpts->id == $subject->id) selected @endif>{{ $subjectOpts->name }}</option>
                    @endforeach
                </select>
            </div>
