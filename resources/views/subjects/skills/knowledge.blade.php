...
@section('content')
    <div class="container">
        <form action="{{ route('subjects.skills.store.knowledge') }}" method="POST">
            @csrf
            <div class="row mb-3">
                <div class="col-6">
                    <label for="subject_id"
                           class="form-label">{{ trans_choice('subjects.skills.subject',1   ) }}</label>
                    <livewire:subject-matter.subject-matter-selector/>
            ...
            <div class="row">
                <button type="submit"
                        class="btn btn-primary col mx-2">{{ __('subjects.skills.knowledge.add') }}</button>
