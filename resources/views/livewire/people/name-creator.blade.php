<div>
            ...
            @foreach($tokens as $token)
                <div class="p-0 m-2" wire:key="{{ $loop->index }}" wire:sortable.item="{{ $loop->index }}">
                    <div class="input-group">
                        ...
                        @else
                            <input type="text" value="{{ $token->textContent }}" class="form-control form-control-sm"
        ...
        <div class="col-2">
            <div class="d-grid gap-1 ms-auto">
                <button type="button" class="btn btn-outline-primary btn-sm text-start"
                        wire:click="addBasicToken()"><span
                            class="border-end me-2 pe-2"><i
                                class="fa fa-plus"></i></span>{{ __('people.name.creator.basic') }}
                </button>
                <button type="button" class="btn btn-outline-primary btn-sm text-start"
                        wire:click="addRoleToken()"><span
                            class="border-end me-2 pe-2"><i
                                class="fa fa-plus"></i></span>{{ __('people.name.creator.role') }}
                </button>
                <button type="button" class="btn btn-outline-primary btn-sm text-start"
                        wire:click="addTextToken()"><span
                            class="border-end me-2 pe-2"><i
    ...
    <div class="alert alert-info mb-3 d-flex justify-content-between align-items-center">
        ...
        <button type="button" class="btn btn-primary"
                wire:click="newSamplePerson()">{{ __('people.name.creator.random') }}</button>
    </div>
    <div class="row">
        <button type="button" class="col mx-2 btn btn-primary"
                wire:click="saveName()">{{ __('common.apply.changes') }}</button>
        <button type="button" class="col mx-2 btn btn-secondary"
                wire:click="resetName()">{{ __('common.revert.changes') }}</button>
    </div>
</div>
