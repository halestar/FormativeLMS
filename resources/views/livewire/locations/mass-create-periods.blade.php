        ...
        <div class="tab-pane fade show active my-3" role="tabpanel">
            @if($tab == 'copy_campus')
                <div class="input-group mb-3">
                    ...
                    <select name="copy_days" id="copy_days" class="form-select" wire:model="copy_day" wire:change="updateCopyPeriods()">
            ...
            @elseif($tab == 'copy_days')
                <div class="input-group mb-3">
                    <label for="copy_from_day" class="input-group-text">{{ __('locations.periods.mass.copy.day.from') }}</label>
                    <select id="copy_from_day" class="form-select" wire:model="copy_from_day" wire:change="updateCopyToDay()">
                        @foreach(\App\Classes\Days::weekdaysOptions() as $dayId => $dayName)
                            <option value="{{ $dayId }}">{{ $dayName }}</option>
                        @endforeach
                    </select>
                    <label for="copy_to_day" class="input-group-text">{{ __('common.to') }}</label>
                    <select id="copy_to_day" class="form-select" wire:model="copy_to_day" wire:change="updateCopyToDay()">
                        <option value="0">{{ __('locations.period.day.all') }}</option>
                        @foreach(\App\Classes\Days::weekdaysOptions() as $dayId => $dayName)
                            <option value="{{ $dayId }}">{{ $dayName }}</option>
                        @endforeach
            ...
            @elseif($tab == 'mass_create')
                <div class="input-group mb-3">
                    ...
                    <select name="copy_days" id="copy_days" class="form-select" wire:model="create_day" wire:change="updateMassCreate()">
                        @foreach(\App\Classes\Days::weekdaysOptions() as $dayId => $dayName)
                            <option value="{{ $dayId }}">{{ $dayName }}</option>
                        @endforeach
                    </select>
                    <label for="create_start" class="input-group-text">{{ __('locations.period.mass.create.starting') }}</label>
                    <input
                        type="time"
                    ...
                    />
                    <label for="create_end" class="input-group-text">{{ __('locations.period.mass.create.ending') }}</label>
                    <input
                        type="time"
                        name="create_end"
                        ...
                        wire:change="updateMassCreate()"
                    />
                    <label for="create_duration" class="input-group-text">{{ __('locations.period.mass.create.duration') }}</label>
                    <input
                        type="number"
                        ...
                        wire:change="updateMassCreate()"
                    />
                    <label for="create_between" class="input-group-text">{{ __('locations.period.mass.create.between') }}</label>
                    <input
                        type="number"
                        name="create_between"
