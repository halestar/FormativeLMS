        ...
        <div class="border-top mt-3 w-100">
            ...
            <div class="form-check mt-3 fs-3">
                ...
                <label class="form-check-label mt-1"
                       for="notify">{{ __('subjects.school.widgets.class-announcements.notify') }}</label>
            </div>
        </div>
    </div>
    @if(!$editing)
        @if($announcement->hasAnnouncement() || $announcement->canManageAnnouncement())
            <div class="top-announcement d-flex justify-content-between align-items-center"
                 style="background-color: {{ $announcement->getAnnouncementColor() }}; color: {{ $announcement->getTextHex() }};">
                <div>
        ...
        tinymce.init({
            ...
            setup: function (editor) {
                ...
                editor.on('change', function () {
                    @this.
                    set('announcementText', editor.getContent());
                });
            }
        });
    });
