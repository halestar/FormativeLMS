            ...
            <div class="schedule-day-column @if($loop->first) first @endif"
                ...
                @foreach($schedule[$dayId] as $event)
                    <div class="schedule-event"
                         style="height: {{ $this->getEventHeight($event['period']) }}px; top: {{ $this->getEventTop($event['period']) }}px; left: 0; width: {{ $hourWidth }}px; background-color: {{ $event['color'] }}; color: {{ $event['text'] }} !important;">
                        <a href="{{ $event['link']?? "#none" }}" class="text-reset text-decoration-none">
                            ...
                            <div class="schedule-event-label"
                                 style="line-height: {{ $this->getEventHeight($event['period']) }}px;">
                                <i>{{ $event['label'] }}</i>
                            </div>
                ...
                @endforeach
                @if($this->hasNow($dayId))
                    <div class="now-bar"
                         style="top: {{ $this->getNowTop() }}px; width: {{ $hourWidth }}px;">{{ __('common.now') }}</div>
                @endif
            </div>
        @endforeach
    </div>
