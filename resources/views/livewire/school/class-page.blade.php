    ...
    <div class="row profile-head-row">
        ...
        <div class="col-md-6">
            <div class="profile-head d-flex align-items-start flex-column h-100">
                ...
                <ul class="nav nav-tabs mt-auto w-100" id="profile-tabs" role="tablist"
                    ...
                    @if($editing)
                        <li class="nav-item">
                            <button type="button" class="btn btn-outline-primary btm-sm" data-bs-toggle="modal"
                                    data-bs-target="#class-modal">
                    ...
                    @foreach($classSession->students as $student)
                        <li class="list-group-item list-group-item-action">
                            ...
                            <a href="{{ route('people.show', ['person' => $student->person_id]) }}"
                               class="fs-5">{{ $student->person->name }}</a>
                        </li>
                    @endforeach
                </ul>
            </div>
                    ...
                    @if($editing)
                        <div class="tab-editor mb-3">
                            <div class="input-group">
                                ...
                                <input type="text" id="edit-tab-name" wire:model="editTabName"
                        ...
                        @foreach($widgets as $widget)
                            <div class="row justify-content-start my-2 g-0" wire:key="{{ $widget->getId() }}">
                                ...
                                <div class="col @if($editing) p-2 border border-black rounded-end rounded-bottom @endif">
                                    <livewire:dynamic-component :is="$widget->getComponentName()"
                                                                :key="$widget->getId()" :classWidget="$widget"
                                                                :canManage="$canManage"/>
                                </div>
                            </div>
                        @endforeach
            ...
            <div class="modal-content">
                <div class="modal-body">
                    <div class="input-group">
                        ...
                        <button type="button" class="btn btn-success" wire:click="addTab()"><i class="fa fa-check"></i>
                        </button>
