    ...
    @if($canManage)
        ...
        <div class="border rounded bg-light p-3 @if(!($editing || $adding)) d-none @endif">
            <div class="mb-3">
                ...
                <input type="text" id="link_text" class="form-control @error('linkText') is-invalid @enderror" wire:model="linkText"/>
            ...
            <div class="mb-3">
                ...
                <input type="text" id="link_url" class="form-control @error('linkUrl') is-invalid @enderror" wire:model="linkUrl"/>
                <x-error-display key="linkUrl">{{ $errors->first('linkUrl') }}</x-error-display>
            </div>
            <div class="row">
                @if($adding)
                    <button type="button" class="btn btn-primary col" wire:click="addLink()">{{ __('common.add') }}</button>
                @else
                        ...
                        @foreach($sessionWidgets as $session)
                            @foreach($session['widgets'] as $w)
                                <div class="form-check form-check-inline">
                                    ...
                                    <label class="form-check-label" for="ow-{{ $w->getId() }}">{{ $w->title }} ({{ $session['session']->name }})</label>
                                </div>
                            @endforeach
                        @endforeach
                    </div>
