    ...
    <div class="card-body h-100">
        <div class="row h-100">
            <div class="col-md-6 col-lg-5 col-xl-4 mb-4 mb-md-0 p-3 h-100 d-flex flex-column">
                <div class="input-group mb-3">
                    <label for="session-select"
                           class="input-group-text">{{ trans_choice('subjects.class',$session->count()) }}</label>
                    <select id="session-select" wire:model="selectedSessionId" class="form-select"
                            wire:change="setSession()">
            ...
            @if($selectedStudent && $session)
                <livewire:school.class-chat :session="$session" :student="$selectedStudent"
