	...
	public function run(): void
	{
		foreach (Person::teachers()->get() as $faculty)
		{
			...
			foreach ($faculty->currentClassSessions as $session)
			{
				...
				if ($skills->isEmpty())
				{
					//assign a random number of skills.
					$campus = $session->course->campus;
					$skills = Skill::forLevels($campus->levels)->active()
					               ->inRandomOrder()
					               ->limit(rand(1, 20))->get();
					$session->course->skills()->attach($skills->pluck('id')->toArray());
				}
				foreach ($session->classCriteria as $criteria)
				{
					$skillIdx = rand(0, $skills->count() - 1);
					LearningDemonstration::factory()
					                     ->count(1)
					                     ->for($faculty, 'owner')
					                     ->hasAttached($skills[$skillIdx], [
						                     'rubric' => $skills[$skillIdx]->rubric, 'weight' => 1,
					                     ])
					                     ->has(
						                     LearningDemonstrationClassSession::factory()
						                                                      ->count(1)
