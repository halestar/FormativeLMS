...
class CampusSeeder extends Seeder
{
	...
	{
		$buildingA = Building::where('id', '1')
		                     ->first();
		$buildingB = Building::where('id', '2')
		                     ->first();
		// make the campus address
		$faxPhone = Phone::factory()
		                 ->create();
		...
		$hs->phones()
		   ->attach($buildingA->phones()
		                      ->first()->id, ['primary' => true]);
		$hs->phones()
		   ->attach($faxPhone->id, ['primary' => false, 'label' => 'Fax Line']);
		$hs->phones()
		   ->attach($admissionPhone->id, ['primary' => false, 'label' => 'Admissions Line']);
		$hs->rooms()
		   ->sync($buildingA->rooms->pluck('id')
		                           ->toArray());
		...
		$es->addresses()
		   ->attach($buildingA->address_id, ['primary' => true]);
		$es->addresses()
		   ->attach($buildingB->address_id, ['primary' => false]);
		$es->phones()
		   ->attach($buildingA->phones()
		                      ->first()->id, ['primary' => true]);
		$es->phones()
		   ->attach($buildingB->phones()
		                      ->first()->id, ['primary' => false, 'label' => '2nd Campus']);
		$es->phones()
		   ->attach($faxPhone->id, ['primary' => false, 'label' => 'Fax Line']);
		$es->phones()
		   ->attach($admissionPhone->id, ['primary' => false, 'label' => 'Admissions Line']);
		$es->rooms()
		   ->sync($buildingB->rooms()
		                    ->inRandomOrder()
		                    ->limit(5)
		                    ->get()
		                    ->merge($buildingB->rooms()
		                                      ->inRandomOrder()
		                                      ->limit(5)
		                                      ->get())
		                    ->pluck('id')
