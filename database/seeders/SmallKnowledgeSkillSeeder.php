...
class SmallKnowledgeSkillSeeder extends Seeder
{
	public Campus $hs;
	public Campus $ms;
	public Campus $es;
	...
	public function importSkills(string $fname, array $subjects)
	{
		if($standards = fopen($fname, 'r'))
		{
			...
			for($i = 5; $i < $description; $i++)
				$catDesignations[] = SkillCategoryDesignation::where('name', $header[$i])
				                                             ->first()->id;
			...
			while($row = fgetcsv($standards))
			{
				...
				foreach($catDesignations as $catDesignation)
				{
					...
					if($cat && $catDesignation)
						$cat->knowledgeSkills()
						    ->attach($skill->id, ['designation_id' => $catDesignation]);
					else
