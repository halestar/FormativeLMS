	...
	public function index(?Subject $subject = null)
	{
		...
		if(!$subject)
		{
			...
			if(SessionSettings::instance()
				...
				$subject = Subject::find(SessionSettings::instance()
				                                        ->get('subjects.subject_id'));
			else
				$subject = SessionSettings::instance()
				                          ->workingCampus()
				                          ->subjects()
				                          ->first();
		}
		...
		SessionSettings::instance()
		               ->set('subjects.subject_id', $subject->id);
		//and save the campus
		SessionSettings::instance()
		               ->workingCampus($subject->campus_id);
	...
	public function edit(Course $course)
	{
		...
		$breadcrumb =
			[
				...
				trans_choice('subjects.course', 2) => route('subjects.courses.index',
