...
class SchoolClassController extends Controller implements HasMiddleware
{
	...
	public function index(Request $request, ?Course $course = null)
	{
		if(!$course)
		{
			...
			if(SessionSettings::instance()
			                  ->has('subjects.course_id'))
				$course = Course::find(SessionSettings::instance()
				                                      ->get('subjects.course_id'));
			elseif(SessionSettings::instance()
			                      ->has('subjects.subject_id'))
				$course = Subject::find(SessionSettings::instance()
		...
		if($request->has('year_id'))
		{
			...
			$year = SessionSettings::instance()
			                       ->workingYear();
		
		//save the course.
		SessionSettings::instance()
		               ->set('subjects.course_id', $course->id);
		//save the subject.
		SessionSettings::instance()
		               ->set('subjects.subject_id', $course->subject_id);
		//and save the campus
		SessionSettings::instance()
		               ->workingCampus($course->campus->campus_id);
	...
	public function store(Course $course, Request $request)
	{
		...
		$schoolClass->year_id = SessionSettings::instance()
		                                       ->workingYear()->id;
		$schoolClass->save();
