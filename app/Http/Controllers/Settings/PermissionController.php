...
class PermissionController extends Controller
{
	...
	public function store(Request $request)
	{
		...
		$permissionData = Arr::except($data, ['roles']) + ['guard_name' => 'web'];
		...
		return redirect()
			...
			->with('success-status', __('settings.permission.created'));
	}
	...
	public function update(Request $request, SchoolPermission $permission)
	{
		...
		$data = $request->validate([
			'category_id' => 'required|numeric|exists:permission_categories,id',
			'name'        => [
				'required', 'max:255', Rule::unique('permissions')
				                           ->ignore($permission),
			],
			'description' => 'required|min:25|max:255',
			'roles'       => 'nullable|array',
		...
		return redirect()
			->route('settings.permissions.index')
			->with('success-status', __('settings.permission.updated'));
	}
	...
	public function destroy(SchoolPermission $permission)
	{
		...
		return redirect()
			->route('settings.permissions.index')
			->with('success-status', __('settings.permission.deleted'));
	}
}
