...
class SchoolSettingsController extends Controller
{
	...
	public function show(IntegrationsManager $integrations)
	{
		...
		return view(
			'settings.school.show',
			compact(
				'breadcrumb', 'studentRole',
	...
	public function update(Request $request, SchoolSettings $settings, AiSettings $aiSettings)
	{
	...
	public function updateAuth(Request $request, AuthSettings $settings)
	{
		$data = $request->validate([
			...
			'mfa_timeout_days'    => 'required|numeric|min:1',
		], static::errors());
		...
		$settings->mfa_force = $request->has('mfa_force');
		$settings->mfa_force_admins = $request->has('mfa_force_admins');
		$settings->mfa_timeout_days = $data['mfa_timeout_days'];
		$settings->save();
