...
class LocalIntegratorController extends Controller
{
	...
	public function classes_update(Request $request)
	{
		...
		foreach ((array)$classesService->data->available as $widgetClass => $widgetName)
		{
			...
			elseif ($data[$widgetClass] == 'optional')
	...
	public function classPreferences(SchoolClass $schoolClass)
	{
		$breadcrumb =
			[
				$schoolClass->currentSession()->name_with_schedule => route('subjects.school.classes.show', $schoolClass->currentSession()),
				__('school.classes.preferences') => '#',
			];
		$service = LocalIntegrator::getService(IntegratorServiceTypes::CLASSES);
		$prefs = Auth::user()->getPreference('classes.local', [
			'enabled' => false,
			'widgets' => [],
		]);

		return view(
			'integrators.local.classes.preferences',
			[
	...
	public function aiPreferences(Request $request)
	{
		$aiService = LocalIntegrator::getService(IntegratorServiceTypes::AI);
		$breadcrumb =
			[
				__('system.menu.integrators') => route('integrators.index'),
	...
	public function aiPreferences_update(Request $request)
	{
		$aiService = LocalIntegrator::getService(IntegratorServiceTypes::AI);
		$data = Validator::make(
			$request->all(),
	...
	public function aiPersonalPreferences()
	{
		$aiService = LocalIntegrator::getService(IntegratorServiceTypes::AI);
		$person = Auth::user();
		$breadcrumb =
			[
				__('people.profile.mine') => route('people.show', Auth::user()->school_id),
				$aiService->name => '#',
			];
		$connection = $aiService->connect($person);

		return view('integrators.local.ai.personal', compact('aiService', 'breadcrumb', 'connection', 'person'));
	}

	public function aiPersonalPreferences_update(Request $request)
	{
		$aiService = LocalIntegrator::getService(IntegratorServiceTypes::AI);
		$data = Validator::make(
			$request->all(),
