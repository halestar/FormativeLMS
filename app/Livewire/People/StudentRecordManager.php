...
class StudentRecordManager extends Component
{
	...
	public function addRecord()
	{
		...
		if($latestRecord)
		{
			...
			$year = Year::where('year_start', '>', $latestRecord->year->year_start)
			            ->first();
			if(!$year)
				$year = $latestRecord->year;
			$level = Level::where('order', '>', $latestRecord->level->order)
			              ->orderBy('order', 'asc')
			              ->first();
			if(!$level)
				$level = Level::first();
		...
		$newRecord->campus_id = $level->campuses()
		                              ->first()->id;
		$newRecord->start_date = $year->year_start;
		$this->person->studentRecords()
		             ->save($newRecord);
		$this->person->refresh();
		$this->refreshRecords();
	}
	...
	public function updateStartDate(StudentRecord $studentRecord, string $startDate)
	{
		...
		{
			$this->addError('startDate-' . $studentRecord->id,
				'Start date must be between the start and end dates of the year.');
		}
		$this->refreshRecords();
	}
	...
	public function updateEndDate(StudentRecord $studentRecord, string $endDate)
	{
		...
		{
			$this->addError('endDate-' . $studentRecord->id,
				'End date must be between the start and end dates of the year.');
		}
		$this->refreshRecords();
	}
