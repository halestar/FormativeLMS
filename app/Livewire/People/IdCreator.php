...
class IdCreator extends Component
{
    ...
    public function selectElement(?int $elementRow, ?int $elementCol)
    {
        ...
        if($elementRow >= 0 && $elementCol >= 0 && $this->idCard->getContent($elementRow, $elementCol) instanceof IdCardElement)
    ...
    public function updateSetting(string $key, mixed $value)
    {
        ...
        {
            $this->idCard->getContent($this->selectedRow, $this->selectedCol)->setConfig($key, $value);
            $this->isSaved = false;
        }
    }
