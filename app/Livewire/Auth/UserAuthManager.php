...
class UserAuthManager extends Component
{
	...
	{
		$this->person = $person;
		$services = IntegrationService::where('service_type', IntegratorServiceTypes::AUTHENTICATION)
			->enabled()
			->get();
		$this->authServices = [];
		foreach ($services as $service)
		{
			if ($service->canConnectToPeople())
	...
	public function applyChangeAuth()
	{
		...
		//either connect of register a connection
		$connection = $newService->connect($this->person);
		if(!$connection)
			$connection = $newService->registerConnection($this->person);
		$this->person->authConnection()->associate($connection);
		$this->person->save();
		$this->changingAuth = false;
		$this->isLocked = false;
	}
