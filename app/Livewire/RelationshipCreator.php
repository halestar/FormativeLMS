...
class RelationshipCreator extends Component
{
	...
	public function setEditing(Person $person)
	{
		...
		$relationship = $this->person->relationships()
		                             ->where('to_person_id', $person->id)
		                             ->first();
		$reverseRelationship = $person->relationships()
		                              ->where('to_person_id', $this->person->id)
		                              ->first();
		if($relationship)
	...
	public function addRelationship()
	{
		$this->person->relationships()
		             ->attach($this->to_person->id, ['relationship_id' => $this->relationship_id]);
		if($this->reciprocal)
			$this->to_person->relationships()
			                ->attach($this->person->id, ['relationship_id' => $this->reciprocal_id]);
		$this->clearForm();
	}
	...
	public function deleteRelationship(int $toPerson, bool $reciprocal = false)
	{
		...
		$this->person->relationships()
		             ->detach($toPerson->id);
		if($reciprocal)
			$toPerson->relationships()
			         ->detach($this->person->id);
		$this->clearForm();
	}
