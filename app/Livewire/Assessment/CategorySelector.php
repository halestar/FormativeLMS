...
class CategorySelector extends Component
{
	...
	{
		$this->childrenCategories = [];
		$this->latestCategories = $this->rootCategories->where('id', $this->selectedRootCategoryId)
		                                               ->first()->subCategories;
		$this->selectedLatestCategoryId = 0;
		$this->selectedCategoryId = $this->selectedRootCategoryId;
	}
	...
	public function selectLatestCategory()
	{
		if($this->selectedLatestCategoryId != 0)
		{
			...
			$children = $this->latestCategories->where('id', $this->selectedLatestCategoryId)
			                                   ->first()->subCategories;
			if($children->count() > 0)
			{
	...
	public function selectChildCategory(int $childPos, int $childId)
	{
		...
		$this->latestCategories = $this->childrenCategories[($childPos - 1)]->where('id', $childId)
		                                                                    ->first()->subCategories;
		$this->selectedLatestCategoryId = 0;
	}
