...
class SkillCategoryBrowser extends Component
{
    ...
    public function createCategory()
    {
        ...
        if(!$this->selectedCategory)
        {
            ...
            $this->dispatch('edit-category', editCategoryId: $newCategory->id)->to(CategoryItem::class);
        }
        else
            $this->dispatch('new-category', parentId: $newCategory->parent_id)->to(CategoryItem::class);
    }
    ...
    public function setEditCategory(?int $editCategoryId)
    {
        if($editCategoryId == null)
        {
            ...
            $this->dispatch('edit-category', editCategoryId: null)->to(CategoryItem::class);

        }
        else
        {
            $this->editingCategory = SkillCategory::find($editCategoryId);
            $this->dispatch('edit-category', editCategoryId: $this->editingCategory->id)->to(CategoryItem::class);
        }
    }
    ...
    public function render()
    {
        ...
        {
            $this->knowledgeResults = KnowledgeSkill::search($this->search)->get()->sortBy('designation');
            $this->characterResults = CharacterSkill::search($this->search)->get()->sortBy('designation');
        }
        else
        {
