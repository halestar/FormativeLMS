...
class SkillCategoryBrowser extends Component
{
	...
	{
		$this->authorize('has-permission', 'subjects.skills');
		$this->breadcrumb = [trans_choice('subjects.skills', 2) => '#'];
	...
	public function render()
	{
		...
		{
			$query = Skill::search($this->search);
			if (count($this->filterLevels) > 0)
			{
				$query->whereIn('levels', $this->filterLevels);
			}
			$this->skills = $query->get();
		}
		else
		{
			$this->skills = null;
