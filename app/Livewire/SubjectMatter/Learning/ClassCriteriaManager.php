...
class ClassCriteriaManager extends Component
{
	...
	public function mount()
	{
		...
		foreach($this->schoolClasses as $schoolClass)
			$this->sessions[$schoolClass->id] = $schoolClass->sessionsTaughtBy($this->faculty)->get();
	...
	public function selectClass(SchoolClass $schoolClass)
	{
		$this->classSelected = $schoolClass->id;
		$this->classCriteria = $schoolClass->classCriteria;
		SessionSettings::set('class-criteria-manager.selected-class', $schoolClass->id);
	}
	...
	public function addCriteria()
	{
		...
		$criteria->save();
		foreach($this->sessions[$this->classSelected] as $session)
	...
	public function copyToClasses(array $classIds)
	{
		...
		foreach($classCriteria as $criteria)
		{
			foreach($classIds as $schoolClassId)
			{
				...
				foreach($this->sessions[$schoolClassId] as $session)
				{
    ...
    {
	    return view('livewire.subject-matter.learning.class-criteria-manager')
