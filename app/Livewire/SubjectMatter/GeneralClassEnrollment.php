...
class GeneralClassEnrollment extends Component
{
	...
	public function mount()
	{
		...
		//terms data
		$this->terms = $this->selectedYear->campusTerms($this->selectedCampus)
		                                  ->get();
		$this->termIds = $this->terms->pluck('id')
		                             ->toArray();
		//subjects
		...
		//classes
		$this->schoolClasses = $this->selectedCourse->schoolClasses($this->selectedYear)
		                                            ->get();
		//students
	...
	public function refreshStudents(): void
	{
		...
		{
			$enrolled = DB::table('class_sessions_students')
			              ->select('student_id')
	...
	{
		foreach($schoolClass->sessions()
		                    ->whereIn('term_id', $this->termIds)
		                    ->get() as $session)
			$session->students()
			        ->attach($studentIds);
		$this->refreshStudents();
