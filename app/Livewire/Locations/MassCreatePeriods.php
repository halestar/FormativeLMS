...
class MassCreatePeriods extends Component
{
	...
	{
		$this->campus = $campus;
		$this->availableCampuses = Auth::user()
		                               ->employeeCampuses()
	...
	public function updateCopyPeriods()
	{
		if($this->copy_day == Days::ALL)
			$periods = $this->campus->periods()
			                        ->active()
			                        ->get();
		else
			$periods = $this->campus->periods($this->copy_day)
			                        ->active()
			                        ->get();
		$this->copyPeriods = [];
	...
	{
		$periods = $this->campus->periods($this->copy_from_day)
		                        ->active()
		                        ->get();
		$this->copyToDay = [];
		$idx = 1;
		...
		while($start->lt($end))
		{
			$this->createPeriods[$idx] =
				[
					...
					'end' => $start->addMinutes($this->create_duration)
					               ->format('H:i'),
				];
