...
class CampusRoomAssigner extends Component
{
    ...
    public function mount(Campus $campus): void
    {
        ...
        $this->room_id = $this->selectedArea->rooms()->first()->id;
        $this->updateContent();
    }

    public function addRoom(): void
    {
        $this->campus->rooms()->syncWithoutDetaching([$this->room_id]);
        $this->updateContent();
    }

    public function removeRoom(Room $room): void
    {
        $this->campus->rooms()->detach($room);
        $this->updateContent();
    }

    public function addBuildingArea()
    {
        $this->campus->rooms()->syncWithoutDetaching($this->selectedArea->rooms->pluck('id')->toArray());
        $this->updateContent();
    }
    ...
    {
        $this->campus->rooms()->detach($this->rooms->where('area_id', $area->id)->pluck('id')->toArray());
        $this->updateContent();
    }

    public function addBuilding(): void
    {
        $this->campus->rooms()->syncWithoutDetaching($this->selectedBuilding->rooms->pluck('id')->toArray());
        $this->updateContent();
    }
    ...
    {
        $this->campus->rooms()->detach($building->rooms->pluck('id')->toArray());
        $this->updateContent();
    }

    public function toggleClassroom($room_id, $isClassroom)
    {
        $this->campus->rooms()->updateExistingPivot($room_id, ['classroom' => $isClassroom]);
        $this->updateContent();
    }

    public function updateLabel($room_id, $label)
    {
        $this->campus->rooms()->updateExistingPivot($room_id, ['label' => $label]);
        $this->updateContent();
    }
