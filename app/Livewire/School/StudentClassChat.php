	...
	private function setupStudent(ClassSession $selectedSession = null)
	{
		...
		foreach($this->sessions as $session)
		{
			if($this->schoolSettings->year_messages == SchoolSettings::TERM)
				$this->unreadMessages[$session->id] = ClassMessage::numUnreadMessages($session, $this->student,
					$this->self);
			else
				$this->unreadMessages[$session->id] = ClassMessage::numUnreadClassMessages($session, $this->student,
					$this->self);
		}
		$this->latestMessage = [];
		foreach($this->sessions as $session)
