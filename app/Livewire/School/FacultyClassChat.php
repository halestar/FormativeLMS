...
class FacultyClassChat extends Component
{
	...
	public function mount(SchoolSettings $settings, int $selectedSessionId = null, string $size = "",
	...
	private function setupSession(StudentRecord $selectedStudent = null)
	{
		...
		foreach($this->students as $student)
		{
			if($this->schoolSettings->year_messages == SchoolSettings::TERM)
				$this->unreadMessages[$student->id] = ClassMessage::numUnreadMessages($this->session, $student,
					$this->self);
			else
				$this->unreadMessages[$student->id] = ClassMessage::numUnreadClassMessages($this->session, $student,
					$this->self);
		}
		$this->latestMessage = [];
		foreach($this->students as $student)
	...
	public function setSession(int $sessionId = null, int $studentId = null)
	{
		if($sessionId)
			...
			$this->session = $this->sessions->where('id', $this->selectedSessionId)
			                                ->first();
		if(!$this->session)
			$this->session = $this->sessions->first();
		if($this->session)
