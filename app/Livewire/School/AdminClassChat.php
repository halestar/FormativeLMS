    ...
    private function setupStudent(ClassSession $selectedSession = null)
    {
        ...
        foreach($this->sessions as $session)
        {
            if($this->schoolSettings->year_messages == SchoolSettings::TERM)
                $this->unreadMessages[$session->id] = ClassMessage::numUnreadMessages($session, $this->selectedStudent, $this->self);
            else
                $this->unreadMessages[$session->id] = ClassMessage::numUnreadClassMessages($session, $this->selectedStudent, $this->self);
        }
        $this->latestMessage = [];
        foreach($this->sessions as $session)
