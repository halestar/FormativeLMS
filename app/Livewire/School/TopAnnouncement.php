...
class TopAnnouncement extends Component
{
	...
	public function mount(TopAnnouncementWidget $announcement)
	{
		...
		if(Auth::user()
		       ->isTeacher() && Gate::allows('manage', $this->announcement->owner->owner))
			$this->otherClasses = Auth::user()->currentClassSessions->where('id', '<>',
				$this->announcement->owner->owner->id);
		else
	...
	public function edit()
	{
		...
		$this->announcementExpiry = $this->announcement->getAnnouncementExpiry()
		                                               ->format('Y-m-d');
