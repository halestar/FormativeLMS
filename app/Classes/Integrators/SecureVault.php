...
class SecureVault
{
	...
	protected function getNamespaceData(string $namespace): array
	{
		...
		if(Storage::disk($this->disk)
		          ->exists($this->vaultFileName($namespace)))
			return json_decode(Storage::disk($this->disk)
			                          ->get($this->vaultFileName($namespace)), true);
		return [];
	}
	...
	protected function storeNamespaceData(string $namespace, array $data): void
	{
		Storage::disk($this->disk)
		       ->put($this->vaultFileName($namespace), json_encode($data));
	}
