...
class SchoolClass extends Model
{
    ...
    {
        return $this->sessions()->where('term_id', $term->id)->first();
    }

    public function students(array $termIds = null): Collection
    {
        $query = StudentRecord::select('student_records.*')->with(['campus', 'year', 'person', 'level'])
            ->join('class_sessions_students', 'class_sessions_students.student_id', '=', 'student_records.id')
        ...
        return $query->groupBy('student_records.id')->get();
    }
    ...
    public function isEnrolled(StudentRecord $student, array $termIds = null): bool
    {
        ...
        if($termIds)
            $query->whereIn('class_sessions.term_id', $termIds);
        $count = $query->where('class_sessions_students.student_id', $student->id)->count();
        if(!$termIds)
            return ($count > 0);
