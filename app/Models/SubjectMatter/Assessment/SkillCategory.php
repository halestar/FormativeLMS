...
class SkillCategory extends Model
{
    ...
    {
        return $this->morphedByMany(KnowledgeSkill::class, 'skill', 'skill_category_designation', 'category_id', 'skill_id')
    ...
    {
        return $this->morphedByMany(CharacterSkill::class, 'skill', 'skill_category_designation', 'category_id', 'skill_id')
            ->withPivot(['designation_id'])
            ->as('info')
            ->using(SkillCategoryDesignation::class);
    }
    ...
    public function canDelete(): bool
    {
        ...
        if($this->subCategories()->count() > 0)
            return false;
        //and no skills
        if($this->knowledgeSkills()->count() > 0 || $this->characterSkills()->count() > 0)
            return false;
        return $canDelete;
    }
}
