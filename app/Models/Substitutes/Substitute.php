...
class Substitute extends Model implements HasCampuses
{
	use Campuseable;
	...
	public function totalSubbedInYear(?Year $year = null, ?SchoolClass $schoolClass = null): int
	{
		...
		if ($schoolClass)
			return $this->subbedSubstituteCampusRequests()
			            ->whereHas('classRequests', fn (
				            Builder $query) => $query->whereIn(
				            'session_id', $schoolClass->sessions->pluck('id')->toArray()
			            ))
			            ->whereBetween('created_at', [$year->start, $year->end])
			            ->count() +
			       $this->person->totalSubbedInYear($year, $schoolClass);
		return $this->subbedSubstituteCampusRequests()
		            ->whereBetween('created_at', [$year->start, $year->end])
		            ->count() +
		       $this->person->totalSubbedInYear($year);
	}
}
